...
import { BookOpen, BarChart2, Settings, GraduationCap, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden" data-testid="dashboard-layout">
        ...
        <div className="px-6 py-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div
              ...
              style={{ backgroundColor: "hsl(228 66% 54%)" }}
            >
              <GraduationCap className="w-4 h-4 text-white" />
            </div>
            <div>
              <h1 className="text-white font-semibold text-sm tracking-tight">FlexiLearn</h1>
              <p className="text-white/40 text-xs">Adaptive Learning</p>
            </div>
          </div>
        </div>
            ...
            {profile ? (
              <div className="space-y-1">
                ...
                <p className="text-green-300/80 text-xs">
                  {formatProfile(profile.neurodivergentProfile)}
                </p>
              </div>
            ) : (
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Top Header */}
        <header
          className="h-14 flex items-center justify-between px-6 border-b border-border bg-white/60 backdrop-blur-sm flex-shrink-0"
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          {/* Right Sidebar — Agent Command Center */}
          <aside
            className="w-72 flex-shrink-0 border-l border-border bg-white overflow-y-auto"
